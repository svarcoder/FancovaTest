...
const SingUp = () => {
	...
	const onSubmitSignIn = (e) => {
		e.preventDefault();
		axios
			.post("https://admin.fanconvo.com/api/v3/sign-up/fan", {
			...
			.catch((err) => {
				...
				if (err?.response?.data?.data) {
					let tempError = Object.entries(err?.response?.data?.data);
					console.log(tempError);
					errorMessage = `${tempError[0][0]} : ${tempError[0][1][0]}`;
				}
				console.log("Err", err?.response?.data?.data);
				toast.error(errorMessage ? errorMessage : "Unknown");
			});
	};
